...
namespace Soul.SqlBatis.Infrastructure
{
	public static class TypeMapper1
	{
		...
		private static Func<IDataRecord, T> CreateEntityConverter<T>(IDataRecord dr)
		{
			...
			if (fields.Count == 1 && GetDataRecordDelegate(entityType) != null || JsonConverter.IsJsonType(entityType))
			{
		...
		private static Expression CreateBindingExpression(Expression parameter,Type memberType,DataRecordField field)
		{
			try
			{
				...
				Expression ifElse;
				var dataRecordDelegate = GetDataRecordDelegate(field.Type);
				...
				{
					if (TypeMapper.TryGetMapper(field.Type, memberType, out Delegate mapper))
					{
					...
					{
						var converter = GetConvertToStringMethod(field.Type);
		...
		private static string CreateKey<T>(List<DataRecordField> fields)
		{
			var entityType = typeof(T);
			if (fields.Count == 1 && GetDataRecordDelegate(entityType) != null || JsonConverter.IsJsonType(entityType))
			{
		...
		private static MethodInfo GetConvertToStringMethod(Type type)
		{
		...
		private static Delegate GetDataRecordDelegate(Type type)
		{
			return DataRecordDelegates.GetOrAdd(type, key =>
			{
